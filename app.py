...
# --- Global variables ---
SCAN_INTERVAL = 60  # Default to 60 seconds (1 minute)
SCAN_MODE = "Intraday"  # Default scan mode
interval_lock = threading.Lock()
mode_lock = threading.Lock()
watchlist_lock = threading.Lock()

# FMI Global variables
fmi_data_lock = threading.Lock()
FMI_SCAN_INTERVAL = 300 # Changed to 300 seconds (5 minutes)
fmi_data = {
    "long_pct": 0, "short_pct": 0, "nifty_signal": "Neutral",
    "nifty_reason": "Initializing...", "last_update": None
}
...
def update_error_status(has_error, error_type="", message=""):
    ...
    with error_status_lock:
        ...
        error_status = {
            ...
            "last_error_time": datetime.now().isoformat() if has_error else None,
            "error_message": message if has_error else ""
        }

def get_momentum_signal_pv(ticker_data, window=FMI_MOMENTUM_WINDOW):
    """
    Determines the momentum signal based on Price and Volume interaction.
    Returns (signal, reason), where signal is 1 for Bullish, -1 for Bearish, 0 for Neutral.
    """
...
def run_fmi_scan():
    """Performs a single FMI scan and updates the global data."""
    try:
        print(f"\n🔄 [{datetime.now().strftime('%I:%M:%S %p')}] Starting FMI calculation...")

        bullish_count, bearish_count = 0, 0

        # 1. Analyze F&O Stocks
        ...
        # 3. Calculate percentages and update global variable
...
def load_fmi_nifty_cache():
    """Load cached NIFTY price data for FMI"""
    try:
...
def save_fmi_nifty_cache(nifty_price):
    """Save NIFTY price cache for FMI"""
    try:
...
def update_fmi_data_periodically():
    """Runs in a background thread to update FMI data every 5 minutes after checking NIFTY price change."""
    last_scan_time = time.time() - FMI_SCAN_INTERVAL # Ensure first scan runs immediately
    while True:
        try:
            ...
            if time_elapsed:
                ...
                if nifty_changed:
                    ...
                    last_scan_time = current_time  # Update timer even if skipped
            else:
                # Print waiting status occasionally
...
def save_historical_data(scan_data, scan_mode):
    """Save scan data for historical comparison, keyed by date."""
    try:
        today_str = datetime.now().strftime('%Y-%m-%d')
        historical_data = load_historical_data()

        # Store data for today with the scan mode
        ...
        # Optional: Keep only the last N days of data to prevent the file from growing too large
        max_days = 10
...
def load_intraday_alerts():
    """Load today's intraday alerts to prevent duplicates"""
    try:
...
def is_fresh_breakout(stock, zone_type, historical_data):
    """Check if this is a fresh breakout (not seen in last 2 days)"""
    try:
        # Check for yesterday
...
def should_trigger_scan():
    """Check if scan should be triggered based on NIFTY price change"""
    try:
        nifty = yf.Ticker("^NSEI")
...
class ZoneScannerWebUI(BaseHTTPRequestHandler):
    ...
    def trigger_fmi_refresh(self):
        """Triggers a manual FMI scan in a new thread."""
        ...
        try:
            if os.path.exists(SUPPLY_BROKEN_FILE):
                ...
                supply_zones = df_supply.rename(columns={
                    ...
                    'Supply Low': 'zone_low', 'Supply High': 'zone_high',
                    'Time': 'time', 'Lot Size': 'lot_size'
                }).to_dict('records')
        except Exception as e:
        ...
        try:
            if os.path.exists(DEMAND_BROKEN_FILE):
                ...
                demand_zones = df_demand.rename(columns={
                    'Stock': 'stock', 'Price': 'price',
                    'Demand Low': 'zone_low', 'Demand High': 'zone_high',
                    'Time': 'time', 'Lot Size': 'lot_size'
                }).to_dict('records')
        except Exception as e:
